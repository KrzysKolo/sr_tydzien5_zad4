...
const UserContextProvider = (props) => {
  ...
  const getUsers = () => {
    ...
    if (searchValue.trim() === "") {
      ...
      users.forEach(user => {
        if( (searchValue.trim() === user.filter((item) => item.firstname.toLowerCase().includes(searchValue.trim()))) || (searchValue.trim() === user.filter((item) => item.lastname.toLowerCase().includes(searchValue.trim()))))
